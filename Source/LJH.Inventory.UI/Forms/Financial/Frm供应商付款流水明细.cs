    ...
    public partial class Frm供应商付款流水明细 : FrmSheetDetailBase
    {
        ...
        protected override void ShowButtonState()
        {
            ...
            else if (PaymentType == CustomerPaymentType.供应商付款)
            {
                ...
                AccountRecord ac = null;
                if (cp != null) ac = new AccountRecordBLL(AppSettings.Current.ConnStr).GetRecord(cp.ID, cp.ClassID).QueryObject;
                btnAssign.Enabled = cp != null && (cp.State == SheetState.新增 || cp.State == SheetState.已审核) && ac != null && ac.Remain > 0 && Operator.Current.Permit(Permission.SupplierPayment, PermissionActions.核销);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (so == SheetOperation.新建 && !string.IsNullOrEmpty(StackSheetID) && !string.IsNullOrEmpty(cp.AccountID)) //如果已经是确定收款是用于哪个出入库单了, 新建时直接核销
            {
