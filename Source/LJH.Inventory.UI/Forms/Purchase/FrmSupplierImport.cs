...
{
    public partial class FrmSupplierImport : Form
    {
        ...
        private CompanyInfo GetDataFromRow(DataGridViewRow row)
        {
            ...
            ret.Name = name;
            ret.ClassID = CompanyClass.Supplier;
                        ...
                        try
                        {
                            if (dataGridView1.Rows[i].Visible)
                            {
                                ...
                                if (info != null)
                                {
                                    ...
                                    {
                                        dataGridView1.Rows[i].Cells["colReason"].Value = "已经存在";
                                    }
                                }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                string modal = System.IO.Path.Combine(Application.StartupPath, "客户导入模板.xls");
                if (File.Exists(modal))
                {
                    SaveFileDialog dig = new SaveFileDialog();
                    dig.Filter = "Excel文档|*.xls;*.xlsx|所有文件(*.*)|*.*";
                    dig.FileName = "供应商导入.xls";
