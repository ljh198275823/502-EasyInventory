...
namespace LJH.Inventory.UI.Forms.Inventory
{
    public partial class FrmSteelRollImport : Form
    {
        ...
        private ProductInventoryItem GetStudentFromRow(DataGridViewRow row)
        {
            ...
            if (row.Cells["colAddDate"].Value != null && !DateTime.TryParse(row.Cells["colAddDate"].Value.ToString(), out addDate))
            {
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
                    ...
                    for (int i = firstRow - 1; i < dataGridView1.Rows.Count; i++)
                    {
                        try
                        {
                            if (dataGridView1.Rows[i].Visible)
                            {
                                ...
                                frm.ShowProgress(string.Format("导入{0}条数据 成功{1}条 失败{2}条", count, success, fail), (decimal)(count) / total);
                            }
                        }
                        catch (Exception ex)
                        {
            ...
            MessageBox.Show(string.Format("共导入{0}条数据 成功{1}条 失败{2}条", count, success, fail), "结果");
        }
        #endregion
    }
}
