...
namespace LJH.Inventory.BLL
{
    public class CustomerTypeBLL : BLLBase<string, CustomerType>
    {
        ...
        {
            List<CustomerType> tps = ProviderFactory.Create<IProvider <CustomerType ,string>>(RepoUri).GetItems(null).QueryObjects;
            ...
            CustomerSearchCondition con = new CustomerSearchCondition() { ClassID = CompanyClass.Customer, Category = info.ID };
            if (sp.GetItems(con).QueryObjects.Count > 0)
            {
                return new CommandResult(ResultCode.Fail, "已经有客户归到此类别，如果确实要删除此类别，请先更改相关客户的所属类别");
            }
            return ProviderFactory.Create<IProvider<CustomerType, string>>(RepoUri).Delete(info);
        }
        #endregion
    }
}
