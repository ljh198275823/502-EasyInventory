...
namespace LJH.Inventory.UI.Forms
{
    public partial class FrmCollectionTypeMaster : FrmMasterBase
    {
        ...
        {
            List<CollectionType> items = null;
            if (SearchCondition == null)
            {
                items = (new CollectionTypeBLL(AppSettings.Current.ConnStr)).GetItems(null).QueryObjects;
            }
            else
            {
                items = (new CollectionTypeBLL(AppSettings.Current.ConnStr)).GetItems(SearchCondition).QueryObjects;
            }
            if (items != null)
            {
